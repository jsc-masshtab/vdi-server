...
sshpass -p ${root_password} ssh ${ssh_opts} root@${controller_ip} ${base_cmd} list ${api_user} ${api_password} -c ${controller_ip}
sshpass -p ${root_password} ssh ${ssh_opts} root@${controller_ip} ${base_cmd} add ${api_user} ${api_password} ${node1_ip} ${node1_ip} ${root_password} -c ${controller_ip}
