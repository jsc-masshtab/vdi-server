...
class PoolLocks:
    ...
    def add_new_pool_data(self, pool_id: str, template_id: str):
        """"При добавлении нового пула добавляем его в _pool_lock_dict. Смотрим есть ли шаблон в _template_lock_dict.
    ...
    async def remove_pool_data(self, pool_id: str, template_id: str):
        """При удаление какого-либо пула отменяем таски, лочим pool lock,
           удаляем пул, убираем его из _pool_lock_dict. Смотрим используется ли шаблон удаляемого пула в других пулах.
           Если не используется, то удаляем его из _template_lock_dict."""
