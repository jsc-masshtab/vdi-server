...
from veil_api_client import VeilClient, VeilClientSingleton, VeilCacheOptions, VeilRetryConfiguration, VeilDomain, VeilTag
...
class VdiVeilClient(VeilClient):
    ...
    async def get(self, api_object, url, extra_params, *args, **kwargs):
        """Переопределен для перехвата окончательного статуса ответов."""
        response = await super().get(api_object=api_object, url=url, extra_params=extra_params, *args, **kwargs)
        ...
        elif hasattr(response, 'status_code') and response.status_code in {401, 403}:
    ...
    async def post(self, api_object, url: str, json_data: dict = None, extra_params: dict = None, *args, **kwargs):
        """Переопределен для перехвата окончательного статуса ответов."""
    ...
    async def put(self, api_object, url: str, json_data: dict = None, extra_params: dict = None, *args, **kwargs):
        """Переопределен для перехвата окончательного статуса ответов."""
        response = await super().put(api_object=api_object, url=url, json_data=json_data, extra_params=extra_params, *args, **kwargs)
        if hasattr(response, 'status_code') and hasattr(api_object, 'api_object_id') and api_object.api_object_id:
            if response.status_code == 404 and isinstance(api_object, VeilDomain):
                from common.models.vm import Vm
                vm_object = await Vm.get(api_object.api_object_id)
...
veil_client = VdiVeilClientSingleton(retry_opts=VeilRetryConfiguration(status_codes={401, 403, 400},
                                                                       timeout=5,
