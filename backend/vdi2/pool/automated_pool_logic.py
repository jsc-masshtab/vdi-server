...
class PoolObject:
    def __init__(self, params: dict):
        self.params = params
        self.write_lock = asyncio.Lock()  # for protection from simultaneous change. its not used yet but should be
    ...
    async def add_domain(self, domain_index):
        ...
        for i in range(MAX_AMOUNT_OF_CREATE_ATTEMPTS):
            ...
            try:
                vm_info = None#await vm.CopyDomain(**params).task
    ...
    async def load_vms(self):
        vms = None#await vm.ListVms(controller_ip=self.params['controller_ip'])
