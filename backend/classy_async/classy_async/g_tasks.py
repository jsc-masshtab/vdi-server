...
#             return '{}.{}'.format(type.__module__, type.__qualname__)
#
...
#                 pass
#             kwargs = ', '.join('{}={}'.format(k, repr(v)) for k, v in info.items())
#             return "{}({})".format(type, kwargs)
#
#         def make_raw(info, result=self.NONE):
#             info = dict(info)
#             call = format_call(info)
#             if result is not self.NONE:
#                 return '{} -> {}'.format(call, repr(result))
#             return call
#
#         li = []
